...
struct CullInstancePass
{
    ...
    ComPtr<ID3D12RootSignature> m_rs_scanPrefix;
    ComPtr<ID3D12PipelineState> m_pso_scanPrefx;
        ...
        auto CreateRS = [&]()
            {
                ...
                ThrowIfFailed( in_device->CreateRootSignature( 0, signature->GetBufferPointer(), signature->GetBufferSize(), IID_PPV_ARGS( &m_rs ) ) );
            };

        auto Create_PSO = [&]( std::wstring shaderPath, ComPtr<ID3D12PipelineState>& pso )
            {
                std::wstring c_csFilename = in_assetPath + shaderPath;
                struct
                {
                    byte* data;
                    uint32_t size;
                ...
                computePsoDesc.pRootSignature = m_rs.Get();
                computePsoDesc.CS = { cshader.data, cshader.size };
                ThrowIfFailed( in_device->CreateComputePipelineState( &computePsoDesc, IID_PPV_ARGS( &pso ) ) );
            };

        CreateRS();
        Create_PSO( L"CullInst.cso", m_pso );
        //Create_PSO( L"ScanGroupSum.cso", m_pso_scanPrefx );
    }
    ...
    {
        in_commandList->SetPipelineState( m_pso.Get() );
        in_commandList->SetComputeRootSignature( m_rs.Get() );
